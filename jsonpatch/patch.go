...
type ApplyOptions struct {
	...
	// SkipMissingPathOnRemove indicates whether to fail "remove" operations when the target path is missing.
	// Default to false.
	SkipMissingPathOnRemove bool
	// SkipMissingPathOnMove indicates whether to fail "move" operations when the target path is missing.
	// Default to false.
	SkipMissingPathOnMove bool
	// SkipMissingPathOnCopy indicates whether to fail "copy" operations when the target path is missing.
	// Default to false.
	SkipMissingPathOnCopy bool
	// SkipMissingPathOnReplace indicates whether to fail "replace" operations when the target path is missing.
	// Default to false.
	SkipMissingPathOnReplace bool
...
func NewApplyOptions() *ApplyOptions {
	return &ApplyOptions{
		...
		SkipMissingPathOnRemove:  false,
		SkipMissingPathOnMove:    false,
		SkipMissingPathOnCopy:    false,
		SkipMissingPathOnReplace: false,
...
func (d *partialDoc) remove(key string, options *ApplyOptions) error {
	_, ok := (*d)[key]
	if !ok {
		if options.SkipMissingPathOnRemove {
			return nil
		}
...
func (p Patch) replace(doc *container, op Operation, options *ApplyOptions) error {
	...
	if ok != nil {
		if options.SkipMissingPathOnReplace {
			return nil
		}
...
func (p Patch) move(doc *container, op Operation, options *ApplyOptions) error {
	...
	if err != nil {
		if options.SkipMissingPathOnMove {
			return nil
		}
...
func (p Patch) copy(doc *container, op Operation, accumulatedCopySize *int64, options *ApplyOptions) error {
	...
	val, err := con.get(key, options)
	if err != nil {
		if options.SkipMissingPathOnCopy {
			return nil
		}
